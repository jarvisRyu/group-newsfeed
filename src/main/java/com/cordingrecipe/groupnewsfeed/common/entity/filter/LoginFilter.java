package com.cordingrecipe.groupnewsfeed.common.entity.filter;
...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (!isWhiteList(requestURI)) { //리스트 조회
            ...
            if (session == null || session.getAttribute("LOGIN_USER") == null) {//세션이없거나 LOGIN_USER에 정보가 없으면
                httpResponse.sendError(HttpServletResponse.SC_UNAUTHORIZED, "로그인 해주세요.");   //401 오류발생
                return;
            }
        }
    ...
    private boolean isWhiteList(String requestURI) {
        return PatternMatchUtils.simpleMatch(WHITE_LIST, requestURI); //요청에 입력된 URI 가 WHTIE_LIST 에 포함되있는지 확인
    }
}
